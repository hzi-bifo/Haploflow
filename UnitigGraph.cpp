...
std::vector<float> UnitigGraph::find_paths()
{
    ...
    while (true)
    {
        ...
        avg /= size;
        unique_paths.push_back(remove_non_unique_paths(unique, visits - 1));
        visits++;
    }
...
float UnitigGraph::remove_non_unique_paths(std::vector<std::vector<UEdge>> unique, unsigned int visits)
{
    float median = 0.f;
