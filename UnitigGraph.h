...
private:
	...
	//UEdge getSeed() const;
	std::vector<float> calculate_thresholds(deBruijnGraph&, std::string, bool);
    std::vector<float> get_thresholds(std::vector<std::map<unsigned int, unsigned int>>& cov_distr, std::string, bool);
